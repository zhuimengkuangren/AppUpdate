...
public class UpdateService extends Service {
    ...
    public void onCreate() {
        Log.e("tag", "UpdateServiec onCreate");
        notificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
        filePath = Environment.getExternalStorageDirectory() + "/AppUpdate/app-debug.apk";
    }

    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.e("tag", "UpdateServiec onstartCommand");
        if (intent == null){
        ...
        UpdateManager.getInstance().startDownloads(apkUrl, filePath, new UpdateDownloadListener() {
            ...
            public void onProgressChanged(int progress, String downloadUrl) {
                ...
                notifyUser(getString(R.string.update_download_progressing),
                        getString(R.string.update_download_progressing),
                        progress);
            }
    ...
    private  PendingIntent getContentIntent(){
        ...
        intent.setDataAndType(Uri.parse("file://" + apkFile.getAbsolutePath()),
                "applicaton/vnd.android.packge-archive");
